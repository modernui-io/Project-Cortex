...
<FeatureGrid>
  <FeatureCard
    ...
    description="ACID stores + Vector index + Facts store + Memory API"
    size="medium"
  />
  <FeatureCard
    icon="shield"
...
- **[Artifacts Operations](/api-reference/artifacts-operations)** - Versioned Documents (v0.33.0+)
  - Interactive, versioned documents for AI-generated content
  - Full undo/redo with version history
  - Streaming support with pause/resume capabilities
  - `create()`, `update()`, `delete()`, `undo()`, `redo()`, `startStreaming()`, `finalizeStreaming()`
...
**Create versioned documents (artifacts):**
→ [Artifacts Operations](/api-reference/artifacts-operations) - `cortex.artifacts.*`
