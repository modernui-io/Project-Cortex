...
```typescript
// Convex provides native vector search via vectorIndex
.vectorIndex("by_embedding", {
  vectorField: "embedding",
  dimensions: 1536,  // Default: text-embedding-3-small
  filterFields: ["memorySpaceId", "tenantId", "userId", "agentId", "participantId"],
...
// Query with vector similarity using ctx.vectorSearch() (must be in an action)
// Returns array of { _id, _score } - fetch full documents separately
...
// Fetch full documents using the returned IDs
...
<DataTable
  headers={["Operation", "Typical Latency", "Indexed", "Scalability"]}
  rows={[
...
<DataTable
  headers={["Operation", "Typical Latency", "ACID", "Versioning"]}
  rows={[
...
<DataTable
  headers={["Operation", "Typical Latency", "Notes"]}
  rows={[
