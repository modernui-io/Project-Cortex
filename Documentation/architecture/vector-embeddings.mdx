...
<DataTable
  headers={["Provider", "Model", "Dimensions", "Max Tokens", "Quality", "Cost"]}
  rows={[
...
<ComparisonTable
  headers={["Dimensions", "Storage per Vector", "Search Speed", "Quality", "Use Case"]}
  items={[
    { feature: "384", values: ["~3KB", "Fastest", "Good", "High-volume, cost-sensitive"] },
    { feature: "768", values: ["~6KB", "Fast", "Good", "Balanced"] },
    { feature: "1024", values: ["~8KB", "Fast", "Excellent", "Cohere, Voyage"] },
    { feature: "1536", values: ["~12KB", "Medium", "Good", "OpenAI standard (DEFAULT)"] },
    { feature: "3072", values: ["~24KB", "Medium", "Excellent", "OpenAI large, best quality"] },
  ]}
/>
...
```typescript
// Query with embedding (SDK handles the vector search internally)
const results = await cortex.memory.search("agent-1", "user password", {
  embedding: await embed("user password"),
  limit: 10,
});

// Internal Convex vector search (must be in an action)
// ctx.vectorSearch() returns { _id, _score }[] - fetch full docs separately
const vectorResults = await ctx.vectorSearch("memories", "by_embedding", {
  vector: queryEmbedding,
  limit: 10,
  filter: (q) => q.eq("memorySpaceId", "agent-1"),
});
...
// Query with pre-filtering (fast!) - must be in an action
const vectorResults = await ctx.vectorSearch("memories", "by_embedding", {
  vector: queryVector,
  limit: 10,
  filter: (q) => q.eq("memorySpaceId", memorySpaceId)  // ← Filtered BEFORE similarity search
