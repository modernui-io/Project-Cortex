...
.vectorIndex("by_embedding", {
  ...
  dimensions: 1536,
  // No filterFields - must filter after search (slow)
})

// Query searches ALL vectors, then filters (slow at scale)
// Note: ctx.vectorSearch() must be called from an action
const results = await ctx.vectorSearch("memories", "by_embedding", {
  vector: vector,
  limit: 10,
  // No filter possible without filterFields
});
// Must manually filter results afterward

// With filterFields: Pre-filter before search (fast!)
.vectorIndex("by_embedding", {
  vectorField: "embedding",
...
const results = await ctx.vectorSearch("memories", "by_embedding", {
  vector: vector,
  limit: 10,
  filter: (q) => q.eq("memorySpaceId", memorySpaceId)  // ← Pre-filtered BEFORE search
...
<DataTable
  headers={["Operation", "Indexed", "Latency (p50)", "Latency (p95)", "Latency (p99)"]}
  rows={[
...
<DataTable
  headers={["Operation", "Latency (p50)", "Latency (p95)", "Throughput"]}
  rows={[
...
<DataTable
  headers={["Dataset", "Agents", "Memories", "Vector Search", "Keyword Search"]}
  rows={[
...
```typescript
// Track query latency - must be an action for vector search
export const search = action({
  ...
  handler: async (ctx, args) => {
    ...
    const filter = args.agentId
      ? (q: any) => q.eq("memorySpaceId", args.memorySpaceId)
