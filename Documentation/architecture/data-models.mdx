...
<DataTable
  headers={["Table", "Layer", "Purpose", "Scoped By", "Versioned", "Retention"]}
  rows={[
...
```typescript
// Semantic search (memorySpace-scoped) - must be in an action
...
// Semantic search (user-specific within space) - must be in an action
const vectorResults = await ctx.vectorSearch("memories", "by_embedding", {
  vector: queryVector,
  limit: 10,
  filter: (q) => q.eq("memorySpaceId", memorySpaceId)
...
<DataTable
  headers={["Table", "Typical Size", "Max Size", "Notes"]}
  rows={[
...
// Query with pre-filter (fast) - must be in an action
const vectorResults = await ctx.vectorSearch("memories", "by_embedding", {
  vector: vector,
  limit: 10,
  filter: (q) => q.eq("memorySpaceId", memorySpaceId)  // ← Filtered BEFORE similarity
...
// Query only searches relevant subset - must be in an action
const vectorResults = await ctx.vectorSearch("memories", "by_embedding", {
  vector: vector,
  limit: 10,
  filter: (q) => q.eq("memorySpaceId", memorySpaceId)  // ← Filters BEFORE similarity
