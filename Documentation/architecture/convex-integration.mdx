...
// Search is an action (not query) because vector search requires ctx.vectorSearch()
export const search = action({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.embedding) {
      // Semantic search using ctx.vectorSearch() (only available in actions)
      ...
      results = await ctx.runQuery(internal.memories.fetchByIds, {
        ids: vectorResults.map(r => r._id),
      });
    } else {
      // Keyword search (can be a query)
      results = await ctx.runQuery(internal.memories.keywordSearch, {
...
- Actions can call external APIs and use ctx.vectorSearch()
- Not reactive (must be explicitly called)
- Good for vector search operations
- Can call internal queries for database access
...
```typescript
// convex/memories.ts
// Vector search must be an action (ctx.vectorSearch only available in actions)
export const semanticSearch = action({
  ...
  handler: async (ctx, args) => {
    ...
    // Note: tenantId, userId, participantId should be in filterFields for efficiency
    let filter = (q: any) => q.eq("memorySpaceId", args.memorySpaceId);

    if (args.tenantId) {
      const prevFilter = filter;
      filter = (q: any) => prevFilter(q).eq("tenantId", args.tenantId);
    }
    ...
    const results = await ctx.runQuery(internal.memories.fetchByIds, {
      ids: vectorResults.map(r => r._id),
    });

    // Post-filter for fields not in filterFields (userId, participantId)
    return results.filter(r => {
      if (args.userId && r.userId !== args.userId) return false;
      if (args.participantId && r.participantId !== args.participantId) return false;
      return true;
    });
  },
});
...
- Must be called from actions (not queries)
