...
```typescript
// Internal Convex action implementation (vector search requires action)
export const semanticSearch = action({
  ...
  handler: async (ctx, args) => {
    ...
    const filter = args.filters.tenantId
      ? (q: any) => q.eq("memorySpaceId", args.memorySpaceId)
                     .eq("tenantId", args.filters.tenantId)
      : (q: any) => q.eq("memorySpaceId", args.memorySpaceId);
    ...
    const docs = await ctx.runQuery(internal.memories.fetchByIds, {
...
```typescript
// Pre-filtered (fast) - must be in an action
const vectorResults = await ctx.vectorSearch("memories", "by_embedding", {
  vector: vector,
  limit: 20,
  filter: (q) => q.eq("memorySpaceId", memorySpaceId)  // ← Pre-filter
...
```typescript
// Post-filter (after vector search) - must be in an action
const vectorResults = await ctx.vectorSearch("memories", "by_embedding", {
  vector: vector,
  limit: 50,  // Get more results for post-filtering
  filter: (q) => q.eq("memorySpaceId", memorySpaceId),
});
...
<DataTable
  headers={["Strategy", "Index Used", "Typical Latency", "Dataset Size Impact"]}
  rows={[
