...
<DataTable
  ...
  rows={[
    ...
    ["`chat-sdk`", "Full-featured chat app based on vercel/ai-chatbot with Cortex Memory"],
  ]}
/>
...
cortex init my-chatbot --template chat-sdk
```
...
<DataTable
  ...
  rows={[
    ["`cortex status`", "Full", "Checks paths + Convex URL accessibility"],
    ["`cortex config list`", "Full", "Checks paths + Convex URL accessibility"],
    ["`cortex config show`", "Full", "Checks paths + Convex URL accessibility"],
    ["`cortex start`", "Path only", "Checks paths only (faster startup)"],
  ]}
/>
...
<Terminal>cortex config remove-deployment &lt;name&gt;</Terminal>
