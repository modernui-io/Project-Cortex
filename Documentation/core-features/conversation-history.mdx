...
<Callout type="tip" title="v0.35.0+">
Shareable Chats add visibility control, sharing grants, snapshots, and collaborative multi-user conversations.
</Callout>

### Visibility Control

Control who can access conversations:
...
// → { canView: true, reason: 'SPACE_MEMBER' }
```

### Sharing Grants

Create shareable links with permissions and limits:
...
// Build shareable URL using the buildShareUrl utility
...
Create immutable copies with optional PII redaction:

```typescript
const snapshot = await cortex.conversations.snapshot({
  conversationId: 'conv-123',
  redactPII: true, // Redacts emails, phones, SSN, etc.
});
...
Multi-user conversations with approval workflow:
...
See [Conversation API Reference](/api-reference/conversation-operations#shareable-chats) for complete documentation.
