...
sidebar_position: 1
description: Understand how Cortex orchestrates memory across four layers with two-phase recall/remember architecture
---
...
Memory orchestration now operates in **two distinct phases**: **Recall** (context retrieval) and **Remember** (memory storage). This enables real-time UI progress indicators and better debugging. See [Two-Phase Architecture](#two-phase-architecture).
</Callout>
...
<Callout type="info" title="New in v0.35.1">
Memory orchestration now operates in two distinct phases, each with its own set of layers.
</Callout>

A complete AI chat turn involves two orchestration phases:

<FlowDiagram title="Two-Phase Orchestration Flow" caption="Recall retrieves context, Remember stores memories">
  <FlowNode title="1. Recall Phase" icon="search" variant="primary">
    memorySpace → user → agent → context
  </FlowNode>
  <FlowNode title="2. LLM Generation" icon="bot" variant="secondary">
    AI generates response using retrieved context
  </FlowNode>
  <FlowNode title="3. Remember Phase" icon="database" variant="success">
...
The **recall** phase runs *before* the LLM generates a response. It retrieves relevant context from memory to inject into the LLM prompt.

**Layers involved:**
- `memorySpace` - Validates memory space exists
- `user` - Loads user profile and preferences
- `agent` - Loads agent configuration
- `context` - **NEW**: Assembles context from memories, facts, and graph

```typescript
// The recall phase is triggered by recall() or automatically before remember()
const result = await cortex.memory.recall({
  memorySpaceId: "user-123-space",
  query: "user preferences",
  observer: {
    onRecallStart: (id) => console.log("Recall starting..."),
    onRecallComplete: (summary) => {
      console.log(`Retrieved ${summary.context.memoriesCount} memories`);
      console.log(`Retrieved ${summary.context.factsCount} facts`);
    },
  },
});

// Use the retrieved context in your LLM call
...
The **remember** phase runs *after* the LLM generates a response. It stores the conversation and extracts new knowledge.

**Layers involved:**
- `conversation` - Stores messages in ACID layer
- `vector` - Creates searchable vector memories
- `facts` - Extracts and stores facts (with belief revision)
- `graph` - Syncs entities to graph database

```typescript
// The remember phase is triggered by remember()
await cortex.memory.remember({
  memorySpaceId: "user-123-space",
  conversationId: "conv-456",
  ...
  userName: "Alex",
  observer: {
    onRememberStart: (id) => console.log("Remember starting..."),
...
Use the `OrchestrationObserver` to monitor both phases in real-time:
...
const observer: OrchestrationObserver = {
  ...
  onRecallComplete: (summary) => {
    hideRecallSpinner();
    displayContextPreview(summary.context);
  },
  ...
  onLayerUpdate: (event) => {
    updateLayerVisualization(event.phase, event.layer, event.status);
  },
};
```

<Callout type="tip">
For Vercel AI SDK apps, use the `useLayerTracking` hook which provides built-in state management for phase-aware layer visualization. See [useLayerTracking Hook](/integrations/vercel-ai-sdk/api-reference#uselayertracking-hook).
</Callout>

**See Also:**
- [Orchestration Observer API](/advanced-topics/orchestration-observer) - Full observer documentation
- [Migration Guide](/advanced-topics/orchestration-observer#migration-guide) - Upgrading from v0.31.x
...
<DataTable
  headers={["Feature", "Default", "What It Does"]}
  rows={[
...
<Callout type="info" title="New in v0.31.0: Configurable Limits">
...
Configure defaults via environment variables (`CORTEX_RECALL_LIMIT_*`) or override per-call. See [Configuration](/getting-started/configuration#recall-limits-configuration).
</Callout>
