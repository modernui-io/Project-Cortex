...
### All Packages v0.36.0 · Jan 26, 2026

**New Chat SDK Template - Production-Ready AI Chat with Memory**

🎉 Complete Vercel AI Chat SDK integration template with persistent memory  
💬 Full-featured chat UI with conversation history, streaming, and attachments  
🧠 Memory-aware responses that improve over time with fact extraction  
🔧 CLI config auto-cleanup with intelligent stale entry detection

<details>
<summary>Show full details</summary>

#### New Chat SDK Template

The new `chat-sdk` template provides a production-ready chat application built on top of the Vercel AI SDK Chat UI components with full Cortex Memory integration:

- **Rich Chat UI**: Message streaming, file attachments, markdown rendering, code highlighting
- **Conversation Management**: Multi-conversation support with sidebar navigation
- **Memory Integration**: Automatic fact extraction and context-aware responses
- **Authentication**: Built-in user authentication with session management
- **Dark Mode**: Full dark mode support out of the box

```bash
# Create a new chat app with memory
cortex init my-chat-app --template chat-sdk
cd my-chat-app
npm run dev
```
...
**Config Auto-Cleanup**: The CLI now automatically validates and removes stale configuration entries:

- Detects deployments with missing project directories
- Validates Convex URLs and removes entries with authentication errors
- Preserves entries during temporary network issues (fail-safe)
- Reassigns default deployment when current default is removed
- Silent cleanup with minimal notification

**Template Sync Support**: Added `--sync-template` support for the chat-sdk template type

**Improved UX**: Spinner indicator during deployment validation (suppresses verbose websocket errors)

#### Dependency Updates

- `ai`: 6.0.49 → 6.0.50
- `@ai-sdk/openai`: 3.0.18 → 3.0.19
- `@ai-sdk/react`: 3.0.51 → 3.0.52
- `next`: 16.1.4 → 16.1.5
- `react`: 19.2.3 → 19.2.4
...
### All Packages v0.35.1 · Jan 26, 2026

**Phase-Aware Memory Orchestration Visibility**

🔍 Distinguish between memory recall (before LLM) and remember (after response)  
📊 Accurate real-time UI feedback showing retrieval vs storage operations  
🆕 New `context` layer for visualizing context assembly during recall
...
- **Phase-aware events**: `LayerEvent` now includes `phase: "recall" | "remember"` field
- **New `context` layer**: Visualize context assembly during recall phase
- **Recall observer support**: `recall()` now accepts an observer parameter for real-time retrieval visibility
- **Separate UI panels**: Memory Recall panel (before LLM) and Memory Storage panel (after response)

#### React Hook Updates

```typescript
const {
  recallLayers,      // Layers during recall phase
  rememberLayers,    // Layers during remember phase
  isRecalling,       // True during recall phase
  isRemembering,     // True during remember phase
  isOrchestrating,   // True if either phase active
  handleDataPart,
} = useLayerTracking();
```
...
Matching updates with snake_case naming convention:
- `on_recall_start`, `on_recall_complete`
- `on_remember_start`, `on_remember_complete`
- `RecallSummary` dataclass with `RecallContext`

</details>

---

### All Packages v0.35.0 · Jan 24, 2026

**Shareable Chats - Visibility, Sharing, Snapshots & Collaboration**

🔓 Conversation visibility controls (private, space, public)  
🔗 Granular sharing grants with permissions, time limits, and view limits  
📸 Immutable conversation snapshots with optional PII redaction  
👥 Multi-user collaborative conversations with message approval workflows
...
**Conversation Visibility (Phase 1)**

Control who can access conversations with three visibility levels:

- **private** (default): Only the conversation owner can access
- **space**: All members of the memory space can access
- **public**: Anyone can access the conversation
...
const conv = await cortex.conversations.create({
  ...
  type: 'user-agent',
  participants: { userId: 'user-123', agentId: 'assistant' },
  visibility: 'space',
});
...
**Sharing Grants (Phase 2)**

Create granular sharing permissions with expiration, view limits, and redaction:

```typescript
// Create a shareable link
const share = await cortex.conversations.share({
  conversationId: 'conv-123',
  grantType: 'link', // or 'user', 'space', 'domain'
...
// Build shareable URL
import { buildShareUrl } from '@cortexmemory/sdk';
const url = buildShareUrl(share.shareId, {
  baseUrl: 'https://myapp.com/shared',
});
```

**Conversation Snapshots (Phase 3)**

Create immutable copies of conversations with PII redaction:

```typescript
const snapshot = await cortex.conversations.snapshot({
  conversationId: 'conv-123',
  redactPII: true, // Redacts emails, phones, SSN, credit cards, IPs
  customRedactions: [
    { pattern: 'ACME Corp', replacement: '[COMPANY]' },
  ],
});
...
Multi-user conversations with message approval workflows:

```typescript
// Create collaborative conversation
const collab = await cortex.conversations.create({
  memorySpaceId: 'team-space',
  type: 'user-agent',
  participants: {
...
- Added `visibility` field to conversations schema
- New `conversationShares` table for sharing grants
- New `conversationSnapshots` table for immutable snapshots
- Added `userIds` and `collaborativeSettings` for collaborative conversations
- Added `approvalStatus`, `approvedBy`, `approvedAt` to messages
- New sharing utility functions: `buildShareUrl()`, `extractShareId()`
- Full Python SDK parity with comprehensive test coverage
- Updated API reference and core features documentation

</details>

---

### All Packages v0.34.0 · Jan 24, 2026

**Multi-Modal Attachments API - File Storage for AI Agents**

📎 New Attachments API for multi-modal file storage (images, audio, video, PDFs)  
☁️ Convex native file storage with signed upload/download URLs  
🔗 Link attachments to conversations, messages, memories, and artifacts  
🐍 Full Python SDK parity with comprehensive test coverage
...
A complete system for multi-modal file storage enabling AI agents to work with images, PDFs, audio, video, and generic files:

- **Upload Workflow**: Generate pre-signed URLs for direct client uploads to Convex storage
- **File Types**: Support for `image`, `audio`, `video`, `pdf`, and generic `file` types
- **Linkage**: Attach files to conversations, messages, memories, or artifacts
- **Metadata**: Store dimensions (images/video), duration (audio/video), and custom metadata
- **Multi-tenancy**: Built-in tenant isolation for SaaS applications
- **Memory Space Scoping**: Files are scoped to memory spaces for proper isolation
...
const images = await cortex.attachments.list({
  memorySpaceId: 'user-123-space',
  type: 'image',
});
```
...
# After uploading file to result.upload_url...
...
- Added `attachments` table to Convex schema with 16 indexes
- New `AttachmentsAPI` class in both TypeScript and Python SDKs
- Comprehensive validation for all attachment parameters
- 258 new unit tests covering all modalities and operations
...
- **Version History**: Track changes with `undo()`, `redo()`, `getHistory()`, and `getVersion()`
- **Streaming Support**: Real-time content streaming with state management (draft → streaming → paused → final)
- **Multi-tenancy**: Built-in tenant isolation for SaaS applications
...
- `createArtifact` - Create artifacts via AI
- `updateArtifact` - Modify existing artifacts
- `appendToArtifact` - Stream content to artifacts
- React hooks: `useArtifacts()` and `useArtifact()`

#### Changes

- Updated `@ai-sdk/openai` to ^3.0.13
- Updated `@ai-sdk/anthropic` to ^3.0.16
- Updated `@types/react` to ^19.2.9
- Updated `framer-motion` to ^12.27.5
...
<DataTable 
  headers={["Component", "Before", "After"]} 
  rows={[
...
- `app/api/chat-v6/route.ts` uses `createLayerStreamObserver()` 
- `lib/layer-tracking.ts` re-exports from `@cortexmemory/vercel-ai-provider/react`
...
Detected patterns:
- `quickstart/` folders with Next.js + `@cortexmemory/vercel-ai-provider`
- Any subdirectory with `@cortexmemory/sdk` dependencies
...
<DataTable 
  headers={["Scenario", "Decision", "Description"]} 
  rows={[
...
<DataTable 
  headers={["Preset", "Required Fields", "Email Validation", "Max Size"]} 
  rows={[
...
<DataTable 
  headers={["Strategy", "Speed", "Accuracy"]} 
  rows={[
