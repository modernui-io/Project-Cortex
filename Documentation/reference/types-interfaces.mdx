...
- [RecallSummary](#recallsummary) - NEW in v0.35.1
...
**Phase-aware orchestration monitoring (v0.35.1+)**
...
- [OrchestrationObserver](#orchestrationobserver) - `onRecallComplete` callback
- [Orchestration Observer Guide](/advanced-topics/orchestration-observer)
 ...
 *
 * Recall phase layers: memorySpace, user, agent, context
 * Remember phase layers: conversation, vector, facts, graph
 */
type MemoryLayer =
  | "memorySpace"
  | "user"
...
interface LayerEvent {
  ...
  /** Time elapsed since phase started (ms) */
  latencyMs?: number;

  /**
   * Phase this event belongs to (NEW in v0.35.1)
   * - "recall": Context retrieval phase (before LLM response)
   * - "remember": Memory storage phase (after LLM response)
   */
  phase: "recall" | "remember";

  /** Data stored/retrieved in this layer (if complete) */
  data?: {
    /** ID of the stored/retrieved record */
    id?: string;
